...
PATCH_MARKER = "V6.22 SINGLE ACTIVE SESSION V1"
...
def install_local_single_session() -> None:
    ...
    def ensure_session_schema() -> None:
        ...
        with lb._connect() as conn:
            with conn.cursor() as cur:
                ...
                # Preserve only the newest historical session per account before
                # enabling the single-session invariant on an upgraded database.
                cur.execute(
        ...
        with lb._connect() as conn:
            with conn.cursor() as cur:
                ...
                cur.execute(
                    ...
                    (digest, normalized_email, lb.session_ttl_seconds(), _client_label(client_info)),
                )
            conn.commit()
    ...
    def change_password(token: str, old_password: str, new_password: str) -> dict[str, Any]:
        ...
        with lb._connect() as conn:
            with conn.cursor() as cur:
                cur.execute("DELETE FROM qlda_local_sessions WHERE email=%s", (str(actor.get("email") or ""),))
            conn.commit()
        return result
