...
PATCH_MARKER = "V6.22 AI IPC CLAIM CONTEXT V1"
MAX_CLAIMS = 80
MAX_MATCH_ITEMS = 250
...
def _claim_appendix(builder, project_id: int, question: str) -> str:
    ...
    with builder.connect() as c:
        ...
        try:
            rows = c.execute(
                """SELECT claim_id,claim_no,claim_code,filename,contractor,contract_no,from_date,to_date,
                          ...
                          disbursement_date,latest_revision,updated_at
        ...
        for row in claims:
            lines.append(
                ...
                f"trạng thái={row.get('payment_status','')} | rev={row.get('latest_revision',0)} | cập nhật={row.get('updated_at','')}"
            )
