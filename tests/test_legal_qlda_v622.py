...
from qlda.runtime_core.legal_pccc_backfill import (
    OFFICIAL_CORE_DOCS,
    PCCC_SYNC_QUERIES,
    clear_pccc_seed_for_tests,
    seed_official_legal_documents,
)
from qlda.runtime_core.legal_qlda import (
...
class LegalQLDAV622Test(unittest.TestCase):
    def test_source_controlled_ui_has_no_draft_controls_and_has_pccc(self):
        source = (ROOT / "src/qlda/presentation/streamlit/app.py").read_text(encoding="utf-8")
        compile(source, "qlda_v76_streamlit.py", "exec")
        self.assertIn("📚 QLXD mở rộng - TVPL", source)
        self.assertIn("🔄 Cập nhật QLXD", source)
        self.assertIn("🔥 PCCC / CNCH", source)
        self.assertIn("seed_official_legal_documents(legal_repo)", source)
        self.assertIn('m4.metric("PCCC / CNCH", pccc)', source)
        self.assertIn("purge_drafts(legal_repo)", source)
        self.assertNotIn("📝 Dự thảo BXD", source)
        self.assertNotIn("Hiển thị cả dự thảo đang lấy ý kiến", source)
        self.assertNotIn('m4.metric("Dự thảo", drafts)', source)
        self.assertIn("legal_repo.list_documents(keyword, category, status, source, False)", source)

    def test_install_expands_qlda_scope_and_sync_all_excludes_drafts(self):
        from qlda.runtime_core import legal_documents as ld
        install_legal_qlda()
    ...
    def test_tt_bxd_family_backfill_recovers_exact_document(self):
        calls = []
        target = {
        ...
        def fake_search(query: str, limit: int = 20):
            ...
            if query == "Thông tư Bộ Xây dựng 2021 TT-BXD":
                return [{"number": "06/2021/TT-BYT", "title": "Thông tư Bộ Y tế", "source_url": "https://example.test/yte", "is_draft": 0}]
            if query in {"/2021/TT-BXD", "2021/TT-BXD", "06/2021/TT-BXD"}:
                return [target, {"number": "07/2021/TT-BXD", "title": "Dự thảo", "source_url": "https://example.test/draft", "is_draft": 1}]
            return []
    ...
    def test_official_seed_guarantees_requested_tt06_and_current_pccc_core(self):
        ...
        for required in (
            "06/2021/TT-BXD", "06/VBHN-BXD", "55/2024/QH15", "105/2025/NĐ-CP",
            "106/2025/NĐ-CP", "69/2026/NĐ-CP", "36/2025/TT-BCA", "63/2025/TT-BXD",
            "QCVN 06:2022/BXD", "QCVN 03:2023/BCA", "QCVN 10:2025/BCA",
            "TCVN 3890:2023", "TCVN 7336:2021", "TCVN 7568-14:2025",
        ):
            self.assertIn(required, numbers)
        tt06 = next(d for d in repo.docs if d.get("number") == "06/2021/TT-BXD")
        self.assertEqual(tt06["source_url"], "https://congbao.chinhphu.vn/van-ban/thong-tu-so-06-2021-tt-bxd-33988.htm")
    ...
    def test_pccc_queries_cover_legal_design_equipment_and_standards(self):
        ...
        for token in ("luật phòng cháy", "xử phạt", "thẩm định thiết kế", "nghiệm thu", "qcvn 06", "qcvn 10", "tcvn", "báo cháy", "chữa cháy tự động"):
    ...
    def test_purge_drafts_removes_old_draft_rows_and_logs_only(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            with sqlite3.connect(db_path) as c:
                c.execute("CREATE TABLE legal_documents(id INTEGER PRIMARY KEY, category TEXT, status TEXT, is_draft INTEGER, title TEXT)")
                c.execute("CREATE TABLE legal_sync_log(id INTEGER PRIMARY KEY, source_name TEXT)")
                c.executemany("INSERT INTO legal_documents(id,category,status,is_draft,title) VALUES(?,?,?,?,?)", [
                ...
                c.executemany("INSERT INTO legal_sync_log(id,source_name) VALUES(?,?)", [(1, "Dự thảo Bộ Xây dựng"), (2, "Thư Viện Pháp Luật")])
