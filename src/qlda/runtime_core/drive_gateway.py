...
class DriveGateway:
    ...
    def _post(self, action: str, payload: dict[str, Any] | None = None, session_token: str = "") -> dict[str, Any]:
        if self.config.local:
            ...
            try:
                from qlda.runtime_core.local_vps_backend import LocalVPSError, dispatch
    ...
    def upload_bytes(
        ...
        *,
        ...
        if self.config.local:
            try:
                from qlda.runtime_core.local_vps_backend import LocalVPSError, save_bytes
                return save_bytes(
                    session_token,
                    project_code=project_code,
                    kind=kind,
    ...
    def download_bytes(self, session_token: str, file_id: str) -> tuple[str, str, bytes]:
        if self.config.local:
            try:
                from qlda.runtime_core.local_vps_backend import LocalVPSError, download_bytes
