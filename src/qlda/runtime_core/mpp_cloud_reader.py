...
from qlda.runtime_core.project_store import planned_progress, calc_progress_status
...
def _ensure_jvm():
    try:
        ...
        raise MppCloudError(
            "Thiếu MPXJ/JPype. VPS/Cloud phải cài mpxj và JPype1 từ requirements.txt."
        ) from exc
        ...
        with _JVM_LOCK:
            if not jpype.isJVMStarted():
                try:
                    ...
                    raise MppCloudError(
                        "Không khởi động được Java JVM. Máy chủ cần default-jre-headless."
                    ) from exc
