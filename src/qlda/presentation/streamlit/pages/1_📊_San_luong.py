...
from qlda.runtime_core.drive_gateway import DriveGateway, config_from_streamlit
...
def _identity() -> dict:
    enforce = str(os.environ.get("QLDA_DRIVE_ENFORCE_RBAC", "true")).strip().lower() in {"1", "true", "yes", "on"}
    if not enforce:
        return {"role": "admin", "email": "", "name": "Admin"}
    token = str(st.session_state.get("qlda_drive_session_token", "") or "").strip()
    if not token:
        st.error("Vui lòng đăng nhập tại trang QLDA chính trước khi mở Sản lượng thi công.")
        st.stop()
    try:
        gateway = DriveGateway(config_from_streamlit(st))
        return dict((gateway.me(token) or {}).get("user") or {})
    except Exception as exc:
        st.error(f"Phiên đăng nhập không hợp lệ: {exc}")
        st.stop()
...
with tab_sources:
    ...
    for source in sources:
        with st.expander(f"{source['name']} • {source.get('spreadsheet_title') or source['spreadsheet_id']}", expanded=False):
            ...
            if c1.button("🔄 Đồng bộ ngay", key=f"sync_{source['source_id']}", type="primary", disabled=not can_update):
                try:
                    ...
                    st.error(str(exc))
            if c2.button("🗑️ Xóa nguồn", key=f"del_{source['source_id']}", disabled=not can_admin):
