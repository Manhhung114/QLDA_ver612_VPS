...
# V6.22 SCHEDULE MANAGEMENT V1
from __future__ import annotations

from qlda.runtime_core.bootstrap import initialize_runtime
initialize_runtime()
from qlda.runtime_core.excel_background import render_boq_background_panel as _v624_render_boq_background_panel
from qlda.runtime_core.excel_background import render_ipc_background_panel as _v624_render_ipc_background_panel
from qlda.runtime_core.excel_background import render_vo_background_panel as _v624_render_vo_background_panel
from qlda.runtime_core.excel_background import render_schedule_background_panel as _v624_render_schedule_background_panel

# V6.22 ORIGINAL IMPORT UI V2
from qlda.runtime_core.original_import_storage import (
...
# V6.22 LOCAL VPS RUNTIME INSTALL V1
import io
import os
import json
import re
...
from qlda.runtime_core.ui_v7_compact import install_theme_v7, install_caption_policy_v7, install_vn_datetime_policy_v7, render_admin_caption_toggle_v7, render_header_v7, render_overview_v7
from qlda.runtime_core.work_tasks_v1 import render_work_tasks_v1
...
LEGAL_CACHE_PATH = DATA_DIR / "legal_cache.json"
...
def _drive_preview_url(file_id: str, file_name: str = "", open_url: str = "") -> str:
    # V6.22 LOCAL VPS UI V1: use the signed URL supplied by the active storage backend.
    direct = str(open_url or '').strip()
    if direct:
        return direct
...
def get_legal_repo(path: str):
    ...
    from qlda.runtime_core.legal_documents import LegalRepository
...
def sidebar_project_tools():
    st.sidebar.markdown("### 🏗️ QLDA Xây dựng")
...
def render_schedule(pid: int):
    ...
    _v624_render_schedule_background_panel(
        st,
        db,
        pid,
    ...
    if _schedule_manage_rows and (_can_update() or _is_admin()):
        with st.expander("⚙️ Quản lý công việc", expanded=False):
            try:
                from qlda.runtime_core.contractor_workspace import list_contractors as _v622_list_contractors
...
def render_document_type(pid: int, doc_type: str):
    ...
    with st.expander("✏️ Thêm / sửa hồ sơ", expanded=False):
...
def render_drawing_type(pid: int, drawing_type: str):
    ...
    with st.expander("✏️ Thêm / sửa bản vẽ", expanded=False):
...
def render_site_diary(pid: int):
    ...
    with st.expander("✏️ Thêm / sửa nhật ký", expanded=False):
...
# V6.22 BOQ MULTISHEET V3 START
from qlda.runtime_core.boq_multisheet import (
...
from qlda.runtime_core.boq_persistence import (
    format_table_number as _v622_format_table_number,
    load_saved_boq_workbook as _v622_load_saved_boq_workbook,
    save_saved_boq_workbook as _v622_save_saved_boq_workbook,
)
...
# V6.22 BOQ MULTISHEET V3 END

# V6.22 IPC CLAIM PAYMENT V1 HELPER
from qlda.runtime_core.ipc_claim_patch import install_ipc_claim_due_date as _v622_install_ipc_claim_due_date
_v622_install_ipc_claim_due_date()
from qlda.runtime_core.original_import_storage import render_ipc_claim_ui_with_original as _v622_render_ipc_claim_ui
from qlda.runtime_core.ipc_claim_period import render_ipc_claim_period_ui as _v622_render_ipc_claim_period_ui
from qlda.runtime_core.ipc_claim_delete import render_ipc_claim_delete_ui as _v622_render_ipc_claim_delete_ui

# V6.22 VO EXCEL V1 HELPER
from qlda.runtime_core.original_import_storage import render_vo_ui_with_original as _v622_render_vo_ui
...
def render_cost_management(pid: int):
    ...
    with tab1:
        ...
        with st.expander("📥 BOQ Excel nhiều sheet", expanded=not bool(rows)):
            st.caption(
                "Workbook BOQ được lưu theo dự án trong cơ sở dữ liệu, dùng chung cho mọi tài khoản có quyền xem dự án. "
                "Số tiền được định dạng dấu phẩy để dễ phân biệt hàng nghìn/triệu/tỷ."
            )

            _saved_boq_result = None
            try:
                _saved_boq_result = _v622_load_saved_boq_workbook(db, pid)
            except Exception as _saved_exc:
                st.warning(f"Chưa đọc được workbook BOQ đã lưu: {_saved_exc}")

            if _saved_boq_result is not None:
                _saved_at = str(_saved_boq_result.get("_persisted_at") or "")
                _saved_name = str(_saved_boq_result.get("filename") or "BOQ.xlsx")
                st.info(
                    f"Đã có workbook BOQ lưu cấp dự án: {_saved_name}"
                    + (f" · cập nhật {_saved_at}" if _saved_at else "")
                    + ". Mở bằng tài khoản khác vẫn xem được dữ liệu này."
                )
            elif rows:
                st.caption(
                    "Dự án đang có các dòng BOQ trong cơ sở dữ liệu nhưng chưa có snapshot workbook nhiều sheet. "
                    "Hãy tải lại file Excel một lần và nhấn Lưu để duy trì workbook cho mọi tài khoản."
                )
            ...
            _v624_render_boq_background_panel(
                st,
                db,
                pid,
                gateway=_drive_gateway(),
            ...
            elif _saved_boq_result is not None:
                _boq_result = _saved_boq_result
                _boq_source = "saved"

            if _boq_result is not None:
                ...
                _msg = (
                    f"{'File đang xem trước' if _boq_source == 'upload' else 'Workbook đã lưu'}: "
                    f"{len(_book_names)} sheet hiển thị, {len(_detail_names)} sheet BOQ chi tiết, "
                    f"{int(_boq_result.get('detail_line_count') or 0):,} dòng BOQ."
                )
                if _summary_name:
                ...
                _m1.metric(
                    ...
                    f"{_v622_format_table_number(_boq_result.get('before_tax_total', 0))} VND",
                )
                _m2.metric(
                    "Thuế VAT",
                    f"{_v622_format_table_number(_boq_result.get('vat_total', 0))} VND",
                )
                _m3.metric(
                    "Cộng giá trị sau thuế",
                    f"{_v622_format_table_number(_boq_result.get('after_tax_total', 0))} VND",
                )
                ...
                if _book_names:
                    ...
                    for _sheet_name, _sheet_tab in zip(_book_names, _sheet_tabs):
                        with _sheet_tab:
                            ...
                            if not _sheet_df.empty:
                                _sheet_df = _sheet_df.map(_v622_format_table_number)
                            st.caption(
                ...
                if _appendix_rows:
                    ...
                    if "TỔNG (VND)" in _appendix_df.columns:
                        _appendix_df["TỔNG (VND)"] = _appendix_df["TỔNG (VND)"].map(_v622_format_table_number)
                    if "STT" in _appendix_df.columns:
                        _appendix_df["STT"] = _appendix_df["STT"].map(_v622_format_table_number)
                ...
                if not _boq_summary.empty:
                    ...
                    for _num_col in ["Số dòng chi tiết", "Giá trị trước thuế (VND)", "Dòng tiêu đề"]:
                        if _num_col in _boq_show.columns:
                            _boq_show[_num_col] = _boq_show[_num_col].map(_v622_format_table_number)
                    st.dataframe(
                ...
                _boq_d2.download_button(
                    ...
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                    key=f"cost_boq_excel_summary_download_{pid}_{_boq_source or 'view'}",
                    width="stretch",
                )

                if _boq_source == "upload":
                    if st.button(
                        "💾 Lưu BOQ + workbook nhiều sheet vào dự án",
                        type="primary",
                        disabled=not _can_update(),
                        ...
                        try:
                            with st.spinner("Đang ghi BOQ chi tiết và workbook vào cơ sở dữ liệu..."):
                                _v622_archive_original_upload(
                            ...
                            st.success(
                                f"Đã lưu {_boq_stats['inserted']:,} dòng BOQ chi tiết và toàn bộ workbook; "
                                f"trước thuế {_v622_format_table_number(_boq_stats['before_tax_total'])} VND; "
                                f"sau thuế {_v622_format_table_number(_boq_stats['after_tax_total'])} VND. "
                                "Dữ liệu được duy trì cấp dự án và dùng chung giữa các tài khoản."
                            )
                            st.rerun()
                        except Exception as _boq_exc:
                            st.error(f"Không thể lưu BOQ/workbook vào database: {_boq_exc}")
                else:
                    st.success(
                        "Workbook này đang được đọc trực tiếp từ cơ sở dữ liệu của dự án, "
                        "không phụ thuộc tài khoản đăng nhập hoặc phiên trình duyệt hiện tại."
                    )
        options=[None]+[int(r["id"]) for r in rows]
        ...
        with st.expander("✏️ Thêm / sửa BOQ", expanded=False):
    ...
    with tab2:
        # V6.22 IPC CLAIM PAYMENT V1
        _v624_render_ipc_background_panel(
            st,
            db,
            pid,
            gateway=_drive_gateway(),
        ...
        _v622_render_ipc_claim_period_ui(db, pid, can_update=bool(_can_update()))
        _v622_render_ipc_claim_delete_ui(db, pid, can_update=bool(_can_update()))

    with tab3:
        # V6.22 VO EXCEL V1
        _v624_render_vo_background_panel(
            st,
            db,
            pid,
            gateway=_drive_gateway(),
...
def render_material_management(pid: int):
    ...
    with tab1:
        ...
        with st.expander("✏️ Thêm / sửa vật tư", expanded=False):
    ...
    with tab2:
        ...
        with st.expander("✏️ Thêm / sửa kế hoạch mua sắm", expanded=False):
    ...
    with tab3:
        ...
        with st.expander("✏️ Thêm / sửa phiếu nhập xuất", expanded=False):
...
# V6.22 REPORT COST V1 HELPER
from qlda.runtime_core.report_cost import render_cost_report as _v622_render_cost_report
...
def render_reports(pid: int):
    ...
    # V6.22 REPORT COST V1
    _v622_render_cost_report(db, pid)
...
def render_legal_documents():
    # V6.22 LEGAL QLXD UI V3 PCCC OFFICIAL BACKFILL
    ...
    from qlda.runtime_core.legal_qlda import install_legal_qlda, purge_drafts
    from qlda.runtime_core.legal_standards_backfill import install_legal_standard_backfill
    from qlda.runtime_core.legal_pccc_backfill import install_legal_pccc_backfill, seed_official_legal_documents
    install_legal_qlda()
    install_legal_standard_backfill()
    install_legal_pccc_backfill()
    from qlda.runtime_core.legal_documents import sync_source, sync_all, search_online_all, search_online_sites
...
def _gateway_logout(preserve_error: bool = False) -> None:
...
def render_settings(master_project_id=None):
        ...
        if not gw.config.configured:
            ...
            try:
                ...
                if ident.get("role") == "admin":
                    ...
                    _ui_note("Chỉ đọc = Viewer Drive • Cập nhật = Viewer Drive + được thêm/sửa/upload qua app, KHÔNG được xóa • Admin = Editor Drive + toàn quyền quản trị/xóa trong app. Phân loại Nhà thầu bắt buộc gán đúng 01 nhà thầu; Chỉ xem toàn bộ nhà thầu luôn là read-only.")
                    ...
                    if users:
                        udf = pd.DataFrame([{
                            "Tên": u.get("name", ""),
                            "Email": u.get("email", ""),
                            "Phân loại duyệt": _v622_approval_labels.get(
...
def render_ai_assistant(pid: int):
    ...
    # V7: chat là tác vụ chính; công cụ phân tích chuyên sâu đóng mặc định.
...
# V6.22 CONTRACTOR WORKSPACE UI V2 SIDEBAR ADMIN START
...
# V6.22 CONTRACTOR SIDEBAR ADMIN INJECT V2
...
if _master_pid and _contractor_ctx and bool(_is_admin()):
    from qlda.runtime_core.contractor_sidebar_admin import _render_admin_tools as _v622_render_contractor_admin_tools
...
# Add / Update / Delete contractor controls are rendered under the selector.
# The temporary management section below is removed by the final sidebar patch
# after contractor access rules are applied. AI/Project keep `_master_pid`.
# V6.22 CONTRACTOR WORKSPACE UI V2 SIDEBAR ADMIN END

# V7 compact operational header. Technical backend/version text stays in Settings.
if not pid:
...
# V7 decision-first navigation: business groups, lazy-rendered one section at a time.
_v7_group_labels = [
    "🏠 Tổng quan",
    "📋 Công việc",
    "🏗️ Thi công",
...
if _v7_group == "🏠 Tổng quan":
    ...
    # Settings and VPS/server metadata are Admin-only. Non-Admin users do not
    # receive these navigation entries at all, not merely a disabled form.
    if _v7_group == "📚 Công cụ" and bool(_is_admin()):
        _v7_sections["📚 Công cụ"].extend([
            ("⚙️ Cài đặt", lambda: render_settings(_master_pid)),
            (
                "🛠️ Hệ thống",
                lambda: render_system_settings_admin(
                    st, db, is_admin=True, actor=_streamlit_user_email()
                ),
            ),
...
# V6.22 CONTRACTOR SIDEBAR FINAL UI V3 ADMIN CONTROLS

# V6.24.5 EXCEL BACKGROUND SOURCE V4

# V7 COMPACT UI SOURCE PATCH V6 CONTRACT MANAGEMENT
