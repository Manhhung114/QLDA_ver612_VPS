...
PATCH_MARKER = "ADVANCED AUTOMATION ADMIN UI V2 COMPACT"
...
def _render_checks(st, db, tenant_id: int, *, actor: str, platform, state_prefix: str) -> None:
    ...
    if c3.button("📈 Dự báo 60 ngày", key=f"{state_prefix}_forecast", use_container_width=True):
        try:
    ...
    if isinstance(contract, dict):
        ...
        m1.metric("Nghĩa vụ mở", int(contract.get("obligation_count") or 0))
    ...
    if isinstance(reconciliation, dict):
        if not reconciliation.get("available", True):
            ...
            m2.metric("Sai lệch", int(reconciliation.get("flag_count") or 0))
    ...
    if isinstance(forecast, dict):
        ...
        m2.metric("Tin cậy", f"{float(forecast.get('confidence') or 0):.0%}")
        m3.metric("Trễ lớn nhất", f"{int(forecast.get('max_predicted_delay_days') or 0)} ngày")
...
def _render_vo(st, db, tenant_id: int, *, actor: str, platform, state_prefix: str) -> None:
    with db.connect() as connection:
        try:
    ...
    manual_rate = v5.number_input(
        "Đơn giá (0 = dò BOQ)", value=0.0, min_value=0.0, key=f"{state_prefix}_vo_rate"
    )
...
def _render_site_vision(st, db, tenant_id: int, *, actor: str, platform, state_prefix: str) -> None:
    ...
    if not images:
        st.info("Chưa có ảnh công trường trong hồ sơ của nhà thầu này.")
        return
    if not task_ids:
        st.info("Chưa có task tiến độ để ánh xạ Site Vision.")
        return
    ...
    if st.button(
        ...
        try:
            ...
            st.success(
                f"AI đề xuất {float(result.get('proposed_progress') or 0):.1f}% · chưa cập nhật tiến độ."
            )
        except Exception as exc:
...
def _render_task_routing(st, db, tenant_id: int) -> None:
    with db.connect() as connection:
        try:
    ...
    if routes:
        ...
        st.info("Chưa có task routing. AI Supervisor sẽ tạo đề xuất khi phát hiện vấn đề cần xử lý.")


def _render_advanced_ui(st, db, tenant_id: int, *, actor: str) -> None:
    """Render compact automation controls inside the dedicated Supervisor page."""
    ...
    check_tab, vo_tab, vision_tab, routing_tab = st.tabs(
        ["Kiểm tra", "Draft VO", "Site Vision", "Task Routing"]
    )

    with check_tab:
        _render_checks(st, db, tenant_id, actor=actor, platform=platform, state_prefix=state_prefix)
    with vo_tab:
        _render_vo(st, db, tenant_id, actor=actor, platform=platform, state_prefix=state_prefix)
    with vision_tab:
        _render_site_vision(st, db, tenant_id, actor=actor, platform=platform, state_prefix=state_prefix)
    with routing_tab:
        _render_task_routing(st, db, tenant_id)


def install_advanced_automation_ui() -> None:
    """Register the compact automation renderer with the Supervisor presentation."""
    ...
    if getattr(overview, "_qlda_advanced_automation_ui_installed", False):
        return
    overview._qlda_advanced_automation_renderer = _render_advanced_ui
    overview._qlda_advanced_automation_ui_installed = True
    overview._qlda_advanced_automation_ui_marker = PATCH_MARKER


__all__ = ["PATCH_MARKER", "_render_advanced_ui", "install_advanced_automation_ui"]
