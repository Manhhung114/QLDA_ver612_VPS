...
"""Status-free meeting-minutes presentation for the frozen V7.6 shell.

This module is a presentation owner. It preserves the existing BBHOP archive
behavior while keeping the retired ``qlda.runtime_core.meeting_minutes_simple``
out of the runtime compatibility layer.
"""
...
def _normalize_existing_rows(db, pid: int, rows: list[Any]) -> list[Any]:
    ...
    for item in rows:
        if any(
            str(_row(item, key, "") or "").strip()
            for key in ("status", "due_date", "closed_date", "priority", "related_wbs")
        ):
...
def render_meeting_minutes_simple(st, app_globals: dict[str, Any], pid: int) -> None:
    ...
    selected = st.selectbox(
        ...
        format_func=lambda value: (
            "➕ Thêm biên bản mới"
            if value is None
            else f"#{value} - {next(str(_row(r, 'code', '')) for r in rows if int(_row(r, 'id', 0)) == value)} · "
                 f"{next(str(_row(r, 'subject', '')) for r in rows if int(_row(r, 'id', 0)) == value)}"
        ),
        key=select_key,
    )
    ...
    with st.expander("📝 Thêm / sửa biên bản họp", expanded=(selected is None)):
        with st.form(f"meeting_minutes_form_{pid}_{selected or 'new'}"):
            ...
            code = c1.text_input(
                "Mã biên bản",
                value=str(_row(record, "code", "")) if record else _next_code(rows),
            )
            subject = c2.text_input(
                "Tên cuộc họp / Nội dung chính *",
                value=str(_row(record, "subject", "")),
            )
            meeting_date = c3.date_input(
                "Ngày họp",
                value=_date_value(_row(record, "issue_date", "")) if record else date.today(),
            )

            c1, c2 = st.columns(2)
            contractor = c1.text_input(
                "Đơn vị / Chủ trì", value=str(_row(record, "contractor", ""))
            )
            issuer = c2.text_input(
                "Người lập biên bản", value=str(_row(record, "issuer", ""))
            )

            attendees = st.text_area(
                "Thành phần tham dự", value=str(_row(record, "assignee", "")), height=90
            )
            description = st.text_area(
                "Nội dung biên bản *", value=str(_row(record, "description", "")), height=260
            )
            response = st.text_area(
                "Kết luận / Công việc sau họp",
                value=str(_row(record, "response", "")),
                height=180,
            )
            note = st.text_area(
                "Ghi chú", value=str(_row(record, "note", "")), height=90
            )
    ...
    if selected:
        ...
        if current:
            ...
            if is_admin and st.button(
                "🗑 Xóa biên bản này", key=f"meeting_minutes_delete_{pid}_{selected}"
            ):
                try:
                    ...
                    if callable(trash_fn):
                        ...
                        if errors:
                            st.error(
                                "Chưa xóa được file trên kho lưu trữ: " + " | ".join(errors)
                            )
                            return
...
def install_meeting_minutes_simple_ui() -> None:
    """Install the status-free meeting-minutes renderer."""
