...
def _sync_dependent_multiselect_state(st, *, key: str, options: list[str], signature_key: str, signature: str) -> None:
    try:
...
def worksheet_catalog(records: Iterable[dict[str, Any]], contractors: Iterable[dict[str, Any]]) -> list[dict[str, Any]]:
...
def _render_overview_all_worksheets(st, service, project_id: int, contractors: list[dict[str, Any]], workspace_ids: list[int]) -> None:
    repo = service.repo
    records = repo.records(project_id, workspace_ids=workspace_ids, limit=20000)
    production = [x for x in records if str(x.get("record_type") or "").upper() == "PRODUCTION"]
    ...
    avg_progress = sum(float(x.get("progress_percent") or 0) for x in production) / len(production) if production else 0.0
    ...
    contractor_options = sorted(x for x in catalog_df["Nhà thầu"].dropna().astype(str).unique().tolist() if x)
    ...
    selected_contractors = f1.multiselect("Nhà thầu", contractor_options, default=contractor_options, key=contractor_key)
    catalog_scope = catalog_df[catalog_df["Nhà thầu"].isin(selected_contractors)].copy()
    worksheet_options = sorted(x for x in catalog_scope["Worksheet"].dropna().astype(str).unique().tolist() if x)
    worksheet_key = f"cdh_overview_worksheets_{int(project_id)}"
    _sanitize_multiselect_state(st, worksheet_key, worksheet_options)
    selected_ws = f2.multiselect("Worksheet", worksheet_options, default=worksheet_options, key=worksheet_key,
        help="Chọn một hoặc nhiều worksheet cần sử dụng. Dữ liệu tiến độ bên phải sẽ tự thay đổi theo lựa chọn này.")
    production_rows: list[dict[str, Any]] = []
    contractor_by_workspace = {int(x.get("workspace_project_id") or 0): dict(x) for x in contractors if int(x.get("workspace_project_id") or 0) > 0}
    for item in production:
        contractor = contractor_by_workspace.get(int(item.get("workspace_project_id") or 0), {})
        production_rows.append({
            "Nhà thầu": _contractor_label(contractor), "Nguồn": item.get("source_name") or "",
            "Worksheet": item.get("worksheet") or "", "Công tác": item.get("work_item") or "",
            "Zone": item.get("zone") or "", "Tiến độ (%)": float(item.get("progress_percent") or 0),
            "Sync": item.get("synced_at") or "",
        })
    production_df = pd.DataFrame(production_rows)
    zone_options: list[str] = []
    if not production_df.empty:
        zone_scope = production_df[production_df["Nhà thầu"].isin(selected_contractors) & production_df["Worksheet"].isin(selected_ws)]
        zone_options = sorted((x for x in zone_scope["Zone"].dropna().astype(str).unique().tolist() if x), key=_progress_dimension_sort_key)
    ...
    _sync_dependent_multiselect_state(st, key=zone_key, options=zone_options, signature_key=zone_signature_key, signature=zone_signature)
    selected_zones = f3.multiselect("Zone / tầng dữ liệu", zone_options, key=zone_key,
        help="Tự động lấy đúng chiều dữ liệu của worksheet đã chọn: Hầm dùng Zone 1/2/3; các sheet dạng ma trận dùng T1, TL, T2, ...")
    ...
    st.caption(f"Đã đồng bộ {len(selected_catalog):,} worksheet · Đã chuẩn hóa {normalized_count:,} · Chưa chuẩn hóa {len(not_normalized):,}.")
    if not not_normalized.empty:
        names = ", ".join(sorted(not_normalized["Worksheet"].astype(str).unique().tolist())[:20])
        st.warning("Các worksheet đã đọc từ Google nhưng chưa nhận diện được dữ liệu sản lượng: " + names + ". Hãy kiểm tra dòng tiêu đề hoặc dữ liệu phần trăm.")
    with st.expander("Trạng thái đồng bộ worksheet", expanded=False):
        ...
        st.dataframe(selected_catalog[visible], hide_index=True, use_container_width=True,
    ...
    if production_df.empty:
        ...
        return
    view = production_df[production_df["Nhà thầu"].isin(selected_contractors) & production_df["Worksheet"].isin(selected_ws) & production_df["Zone"].isin(selected_zones)].copy()
    if view.empty:
        st.info("Không có dữ liệu sản lượng chuẩn hóa phù hợp bộ lọc.")
        return
    pivot = view.pivot_table(index=["Nhà thầu", "Worksheet", "Công tác"], columns="Zone", values="Tiến độ (%)", aggfunc="mean")
    ordered_columns = [x for x in selected_zones if x in pivot.columns]
    if ordered_columns: pivot = pivot.reindex(columns=ordered_columns)
    ...
    attention = view[view["Tiến độ (%)"] < 100].sort_values(["Tiến độ (%)", "Nhà thầu", "Worksheet", "Công tác"])
