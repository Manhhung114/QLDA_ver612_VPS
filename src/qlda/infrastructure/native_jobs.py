...
"""Native V7.1 durable Excel job queue over the existing PostgreSQL schema."""
...
from qlda.infrastructure.native_files import get_file_row
from qlda.infrastructure.postgres import connect

PATCH_VERSION = "V7.1 NATIVE EXCEL JOB QUEUE"
...
def ensure_job_schema() -> None:
    """Create/upgrade the same queue schema used since V6.24."""
...
def _normalize_job_type(value: Any) -> str:
    ...
    return "BOQ" if kind == "BOQ_IMPORT" else kind


def _build_upload_purpose(job_type: str, project_id: int, **options: Any) -> str:
    kind = _normalize_job_type(job_type)
    if kind not in ALLOWED_JOB_TYPES:
        raise ValueError(f"Loại Excel job không hỗ trợ: {kind}")
    project = int(project_id)
    workspace = int(options.pop("workspace_project_id", None) or project)
...
def _parse_upload_purpose(value: str) -> dict[str, Any] | None:
...
def _resource_limits() -> dict[str, float]:
    try:
...
class NativeJobAdapter:
    """JobPort implementation with no qlda.services or versioned legacy imports."""

    def ensure_schema(self) -> None:
        ensure_job_schema()

    def build_upload_purpose(self, job_type: str, project_id: int, **options: Any) -> str:
        return _build_upload_purpose(job_type, project_id, **options)

    def parse_upload_purpose(self, value: str):
        return _parse_upload_purpose(value)

    def enqueue(self, **kwargs: Any):
        ensure_job_schema()
        project_id = int(kwargs.get("project_id") or 0)
        workspace_project_id = int(kwargs.get("workspace_project_id") or project_id)
        file_id = str(kwargs.get("file_id") or "")
        kind = _normalize_job_type(kwargs.get("job_type"))
        if kind not in ALLOWED_JOB_TYPES:
            raise ValueError(f"Loại Excel job không hỗ trợ: {kind}")
        if project_id <= 0 or workspace_project_id <= 0 or not file_id:
            raise ValueError("Thiếu project/workspace/file cho Excel job.")
        ...
        max_attempts = max(1, min(int(kwargs.get("max_attempts") or 2), 5))
        ...
        with connect() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    ...
                    (workspace_project_id, kind, kind, sha),
                )
    ...
    def enqueue_from_upload_purpose(
        self, upload_purpose: str, *, file_id: str, created_by: str = ""
    ):
        meta = _parse_upload_purpose(upload_purpose)
        if not meta:
            return None
        return self.enqueue(
    ...
    def list(self, project_id: int, *, job_type: str = "", limit: int = 10):
        ...
        with connect() as conn:
            with conn.cursor() as cur:
                clauses = ["workspace_project_id=%s"]
                ...
                params.append(count)
                cur.execute(
                    f"SELECT * FROM qlda_excel_jobs WHERE {' AND '.join(clauses)} "
                ...
                cur.execute(
                    """UPDATE qlda_excel_jobs
                       SET status='RUNNING',progress=GREATEST(progress,1),stage='Đang khởi tạo',
                           worker_id=%s,attempts=attempts+1,
    ...
    def request_cancel(self, job_id: int):
        with connect() as conn:
            ...
            conn.commit()
        return {"changed": changed, "job_id": int(job_id)}

    def cancel_requested(self, job_id: int) -> bool:
        with connect() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    "SELECT cancel_requested FROM qlda_excel_jobs WHERE id=%s",
                    (int(job_id),),
                )
                row = cur.fetchone()
    ...
    def update_progress(
        self, job_id: int, progress: int, stage: str, current_sheet: str = ""
    ):
        ...
        with connect() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    """UPDATE qlda_excel_jobs
                       SET progress=%s,stage=%s,current_sheet=%s,heartbeat_at=NOW()
                       WHERE id=%s AND status='RUNNING'""",
                    (
                        pct,
                        str(stage or "Đang xử lý")[:240],
                        str(current_sheet or "")[:240],
                        int(job_id),
                    ),
                )
            conn.commit()
    ...
    def fail(self, job_id: int, error_message: str, *, retry: bool = False):
        message = str(error_message or "Lỗi xử lý Excel")[:4000]
        with connect() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    "SELECT attempts,max_attempts,cancel_requested FROM qlda_excel_jobs WHERE id=%s",
                    (int(job_id),),
                )
                row = cur.fetchone()
                if not row:
            ...
            conn.commit()
        return {"status": status, "job_id": int(job_id)}
                ...
                cur.execute(
                    """UPDATE qlda_excel_jobs
                       SET status=CASE WHEN attempts < max_attempts THEN 'QUEUED' ELSE 'FAILED' END,
                           stage=CASE WHEN attempts < max_attempts
                                      THEN 'Khôi phục sau khi worker gián đoạn' ELSE 'Thất bại' END,
                           worker_id='',current_sheet='',
                           error_message=CASE WHEN attempts < max_attempts THEN error_message
                                      ELSE 'Worker gián đoạn quá số lần cho phép' END,
    ...
    def _worker_states(self) -> list[dict[str, Any]]:
        ...
        with connect() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    "SELECT * FROM qlda_excel_worker_state ORDER BY heartbeat_at DESC"
                )
                rows = cur.fetchall()
        return [_public(row) for row in rows]
    ...
    def queue_stats(self):
        ...
        return {
            ...
            "counts": counts,
            "memory_used_percent": round(_memory_used_percent(), 1),
    ...
    def heartbeat(
        ...
        with connect() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    """INSERT INTO qlda_excel_worker_state(
                           worker_id,hostname,pid,status,current_job_id,heartbeat_at,version)
